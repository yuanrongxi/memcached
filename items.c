...
void do_item_update(item* it)
{
	...
	if(it->time < current_time - ITEM_UPDATE_INTERVAL){ //上次时刻和这次时刻相差不到1分钟，就不改写？
		assert(it->it_flags & ITEM_SLABBED == 0);

		mutex_lock(&cache_lock);
		if((it->it_flags & ITEM_LINKED) != 0){ //重新更新生命周期，防止过期
			item_unlink_q(it);
			it->time = current_time;
			item_link_q(it);
		}
...
//统计各个slab class对item的回收的次数(在本slab class空间不足的时候，会对本slab class中一些存在的slab做回收)
...
item* do_item_get(const char* key, const size_t nkey, const uint32_t hv)
{
	...
	if(it != NULL){
		...
		//在rebalance的内存空间范围中,直接删除掉item关联，为了更快的rebalance
	...
	if(it != NULL){
		if(settings.oldest_live != 0 && settings.oldest_live <= current_time && it->time <= settings.oldest_live){ //超过cache系统的生命周期
			do_item_unlink(it, hv);
			do_item_remove(it);
			it = NULL;
			if (was_found)
		...
		else if(it->exptime != 0 && it->exptime <= current_time){ //kv本身的生命周期结束
			do_item_unlink(it, hv);
			do_item_remove(it);
			it = NULL;
			if (was_found)
